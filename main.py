...
def algo_over_all_dataset(alpha=0.85, n_iter=1000):
    ...
    for name, filepath in zip(datasets[:], files[:]):
        ...
        # Initialize variables for storing run times
        runtimes = []

        # Timing `Standard_PageRank`
        start = time.time()
        std_rank, std_l2 = Standard_PageRank(G_sp, int2node_sp, alpha, n_iter).run()
        end = time.time()
        ...
        # Timing `networkx Simple` PageRank
        start = time.time()
        nx_pagerank_sim = nx.pagerank(G_sp, alpha=1, max_iter=n_iter)
        end = time.time()
        nx_time = end - start
        ...
        # Timing `networkx Standard` PageRank
        start = time.time()
        nx_pagerank_std = nx.pagerank(G_sp, alpha=alpha, max_iter=n_iter)
        end = time.time()
        nx_time = end - start
        ...
        # Timing `networkx Weighted` PageRank
        start = time.time()
        nx_pagerank_wtd = nx.pagerank(nx.from_numpy_array(W, create_using=nx.DiGraph), alpha=alpha, max_iter=n_iter, weight='weight')
        end = time.time()
        nx_time = end - start
        ...
        top_5_nx_sim_rank = sorted(nx_rank_sim, key=lambda x: x[1], reverse=True)[:top_k]
        top_5_nx_std_rank = sorted(nx_rank_std, key=lambda x: x[1], reverse=True)[:top_k]
        top_5_nx_wtd_rank = sorted(nx_rank_wtd, key=lambda x: x[1], reverse=True)[:top_k]

        top_5_standard = get_top_n_ranks(std_rank, int2node_sp, top_k)
        top_5_weighted = get_top_n_ranks(w_rank, int2node_sp, top_k)
        top_5_simplified = get_top_n_ranks(sim_rank, int2node_sp, top_k)

        # Header for the table
        print(f"Dataset - Top {top_k} Ranks Comparison")
        print(f"{'Rank':<5} {'Standard':<25} {'Weighted':<25} {'Simplified':<25} {'NetworkX (Simple)':<25} {'NetworkX (Standard)':<25} {'NetworkX (Weighted)':<25}")
        print('-' * 135)

        # Display top 5 for each algorithm
        for i in range(top_k):
            standard = f"{top_5_standard[i][0]}: {top_5_standard[i][1]:.8f}"
            weighted = f"{top_5_weighted[i][0]}: {top_5_weighted[i][1]:.8f}"
            simplified = f"{top_5_simplified[i][0]}: {top_5_simplified[i][1]:.8f}"
            nx_sim = f"{top_5_nx_sim_rank[i][0]}: {top_5_nx_sim_rank[i][1]:.8f}"
            nx_std = f"{top_5_nx_std_rank[i][0]}: {top_5_nx_std_rank[i][1]:.8f}"
            nx_wtd = f"{top_5_nx_wtd_rank[i][0]}: {top_5_nx_wtd_rank[i][1]:.8f}"
            print(f"{i + 1:<5} {standard:<25} {weighted:<25} {simplified:<25} {nx_sim:<25} {nx_std:<25} {nx_wtd:<25}")
        print('-' * 135)
